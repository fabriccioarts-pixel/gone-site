...
void main() {
  ...
  // Centered at 0,0. The smaller dimension is mapped to -0.5 to 0.5.
  float minRes = min(u_resolution.x, u_resolution.y);
  vec2 uv = (gl_FragCoord.xy - 0.5 * u_resolution.xy) / minRes;
  
  // Pixelization logic
  float pxSize = u_pxSize;
  vec2 pxCoord = gl_FragCoord.xy - 0.5 * u_resolution.xy;
  vec2 pixelizedPxCoord = floor(pxCoord / pxSize) * pxSize;
  vec2 shape_uv = pixelizedPxCoord / minRes;
  
  // Dithering coordinates
  vec2 dithering_uv = floor(gl_FragCoord.xy / pxSize);
  vec2 ditheringNoise_uv = gl_FragCoord.xy;

  float shape = 0.0;
  
  // Shape calculations (using shape_uv)
  if (u_shape < 1.5) {
    // Simplex noise (adjust scale for normalized UV)
    shape = 0.5 + 0.5 * getSimplexNoise(shape_uv * 1.5, t);
    ...
    shape = .15 / abs(sin(t - p.y - p.x));
    shape = smoothstep(0.02, 1., shape);

  } else if (u_shape < 3.5) {
    // Dots
    vec2 p = shape_uv * 20.0;
    float stripeIdx = floor(2.0 * p.x / TWO_PI);
    float rand = hash11(stripeIdx * 10.0);
    rand = sign(rand - .5) * pow(.1 + abs(rand), .4);
    shape = sin(p.x) * cos(p.y - 5.0 * rand * t);
    shape = pow(abs(shape), 6.0);

  } else if (u_shape < 4.5) {
    // Sine wave
    vec2 p = shape_uv * 4.0;
    float wave = cos(.5 * p.x - 2. * t) * sin(1.5 * p.x + t) * (.75 + .25 * cos(3. * t));
    shape = 1. - smoothstep(-0.2, 0.2, p.y + wave);

  } else if (u_shape < 5.5) {
    // Ripple
    float dist = length(shape_uv);
    float waves = sin(dist * 10.0 - 3. * t) * .5 + .5;
    shape = waves;

  } else if (u_shape < 6.5) {
    // Swirl
    float l = length(shape_uv);
    float angle = 2.0 * atan(shape_uv.y, shape_uv.x) + 4. * t;
    float twist = 1.5;
    float offset = pow(l, -twist) + angle / TWO_PI;
    float mid = smoothstep(0., 0.8, pow(l, twist));
...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (locations.u_colorBack) context.uniform4fv(locations.u_colorBack, hexToRgba(colorBack || "#000000"))
      if (locations.u_colorFront) context.uniform4fv(locations.u_colorFront, hexToRgba(colorFront || "#ffffff"))
      if (locations.u_shape) context.uniform1f(locations.u_shape, DitheringShapes[shape || "simplex"])
      if (locations.u_type) context.uniform1f(locations.u_type, DitheringTypes[type || "8x8"])
      if (locations.u_pxSize) context.uniform1f(locations.u_pxSize, pxSize * (window.devicePixelRatio || 1))
